...
public class TeacherFragment extends HomeBaseFragment {
    ...
    public void onEvent(EventMessage msg){
        waitDialog.dismiss();
        if(msg.getMsg().equals("teacherList")){
            adapter.notifyDataSetChanged();
        }else if(msg.getMsg().equals("teacher_search")){
            refreshData(getSearchContent(getContext()));
        }else if(msg.getMsg().equals("teacherList_noData")){
            errorDialog.show();
            ...
            refreshData("");
        } else if(msg.getMsg().equals("teacherList1_more")){
            mToast.showText(getContext().getApplicationContext(),Msg);
        } else if(msg.getMsg().equals("teacherList_error")){
            errorDialog.show();
    ...
    protected void initView(View view) {
        ...
        adapter=new HomeTeacherFragmentAdapter(getContext(),R.layout.item_home_findt_layout,data);
    ...
    private void loadMore(final String page, final int more){
        waitDialog.show();
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                values.add("1");
                values.add(page);
                sendRequest(more,params,values, HomeInterface.teacherList(),"GET");
            }
        }).start();
    }
    ...
    private void refreshData(final String search){
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                values.add("1");
                values.add(search);
                sendRequest(1,params,values, HomeInterface.teacherList(),"GET");
            }
        }).start();
    }
                    ...
                    switch (what){
                        ...
                        case 2:
                            if(object.getString("code").equals("200")){
                                ...
                                Msg="没有更多了";
                                EventBus.getDefault().post(new EventMessage("teacherList1_more"));
                            }
                            break;
                    }
